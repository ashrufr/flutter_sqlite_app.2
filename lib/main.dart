...
class DivineNamesApp extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return MaterialApp(
      ...
      home: const AppHomeScreen(), // Changed to AppHomeScreen
      routes: {
...
class AppHomeScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return DefaultTabController(
      length: 3, // Number of tabs increased to 3
      child: Scaffold(
        appBar: AppBar(
...
class AllNamesListScreen extends StatefulWidget {
  const AllNamesListScreen({Key? key}) : super(key: key);

  @override
  State<AllNamesListScreen> createState() => _AllNamesListScreenState();
}

class _AllNamesListScreenState extends State<AllNamesListScreen> {
  final DivineNamesDatabase _databaseHelper = DivineNamesDatabase();
  ...
  Future<void> _loadAllNames() async {
    ...
    try {
      final names = await _databaseHelper.getAllNames();
      setState(() {
        _namesList = names;
        _isLoading = false;
      });
      ...
      setState(() {
        _errorMessage = 'Error loading names: $e';
        _isLoading = false;
      });
    }
  }
              ...
              : ListView.builder(
                  ...
                  itemBuilder: (context, index) {
                    ...
                    return Card(
                      ...
                      child: ListTile(
                        title: Text(
                          name[DivineNamesDatabase.COLUMN_ENGLISH_NAME] ?? '',
                          style: const TextStyle(fontWeight: FontWeight.w500),
                        ),
                        subtitle: Text(name[DivineNamesDatabase.COLUMN_ARABIC_NAME] ?? ''),
                        onTap: () {
                          // TODO: Implement navigation to details page for the selected name
                          // For now, just print the name to the console
                          print('Tapped on ${name[DivineNamesDatabase.COLUMN_ENGLISH_NAME]}');
                        },
                      ),
                    );
                  },
...
class _FlashcardsScreenState extends State<FlashcardsScreen> {
  ...
  void _nextCard() {
    setState(() {
      ...
      if (_currentCardIndex < _namesList.length - 1) {
        _currentCardIndex++;
      } else {
        _currentCardIndex = 0; // Loop back to the beginning
      }
    });
  }
                      ...
                      child: SizedBox(
                        ...
                        child: Padding(
                          ...
                          child: Column(
                            ...
                            children: [
                              Text(
                                'Card ${_currentCardIndex + 1} of ${_namesList.length}',
                                style: TextStyle(
                                  fontSize: 16,
                                  color: Colors.grey[600],
                                ),
                              ...
                              AnimatedSwitcher(
                                ...
                                child: _isFlipped
                                    ? Text(
                                        '${_namesList[_currentCardIndex][DivineNamesDatabase.COLUMN_ARABIC_NAME] ?? ''}\n\n${_namesList[_currentCardIndex][DivineNamesDatabase.COLUMN_DESCRIPTION] ?? ''}',
                                        key: ValueKey<bool>(_isFlipped), // Important for the animation
                                        style: const TextStyle(fontSize: 18, fontWeight: FontWeight.w500),
                                        textAlign: TextAlign.center,
                                      )
                                    : Text(
                                        _namesList[_currentCardIndex][DivineNamesDatabase.COLUMN_ENGLISH_NAME] ?? '',
                                        key: ValueKey<bool>(_isFlipped), // Important for the animation
